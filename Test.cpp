...
Rect chooseBox(vector <pair<Rect, float>> predicted_boxes) {
	...
	float threshold = 0.77; // Threshold for selecting IoU of each pair predicted boxes to caculate other_predict
	...
	for (int i = 0; i < n; i++) {
		...
		else other_predict[i] = predicted_boxes[i].second;
		// Caculates confidence of i th predicted box 
		float bias = (float) 0.75;
...
Rect polypsDetection(String &path, Ptr<SVM> &svm, vector<pair<int, int>> &anchor) {
	...
	//medianBlur(image_source, image_source, 5);
...
void evaluate_detection(const String& image_dir, Ptr<SVM> &svm, vector<pair<int, int>> &anchor, const String& csv_file) {
...
int main() {
	...
	//vector<pair<int, int>> anchor = { {50, 100}, {75, 100}, {100,50}, {100, 75}, {100, 100} };
	vector<pair<int, int>> anchor = { {40, 100}, {100,40}, {100, 100} };
	//vector<pair<int, int>> anchor = { {100, 40} };
	// Load trained model
	...
	// Load test image to dectect polyps
	String path = "original_data\\994.jpg";
	Rect actual_box = actual_boxes[994];
	...
	cout << IoU(actual_box, predicted_box) << endl;
	...
	*/
	...
	evaluate_detection("original_data", svm, anchor, csv_file);
	cin.ignore();
	waitKey();
	return 0;
}
