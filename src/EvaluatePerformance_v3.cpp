...
vector<Rect> chooseBox(vector <pair<Rect, float>> predicted_boxes) {
	...
	float threshold = 0.1; // Threshold for selecting IoU of each pair predicted boxes to caculate other_predict
	...
	for (int i = 0; i < min(int(temp.size()), 5); i++) {
		if (max < temp[i].first.width) {
			max = temp[i].first.width;
			temp_box = temp[i].first;
		}
	}
	result.push_back(temp_box);
...
vector<Rect> polypsDetection(String &path, Ptr<SVM> &svm, vector<pair<int, int>> &anchor) {
...
int main() {
	...
	// Load test image to dectect polyps
	String path = "original_data\\19.jpg";
	Rect actual_box = actual_boxes[19];

	// Show polyps detection result
	vector<Rect> predicted_boxes = polypsDetection(path, svm, anchor);
